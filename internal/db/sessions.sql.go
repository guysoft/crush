...
INSERT INTO sessions (
    ...
    strftime('%s', 'now'),
    strftime('%s', 'now')
) RETURNING id, parent_session_id, title, message_count, prompt_tokens, completion_tokens, cost, updated_at, created_at, summary_message_id, todos, current_agent_id
`
...
SELECT id, parent_session_id, title, message_count, prompt_tokens, completion_tokens, cost, updated_at, created_at, summary_message_id, todos, current_agent_id
FROM sessions
...
const setSessionCurrentAgent = `-- name: SetSessionCurrentAgent :exec
UPDATE sessions
SET current_agent_id = ?
WHERE id = ?
`

type SetSessionCurrentAgentParams struct {
	CurrentAgentID sql.NullString `json:"current_agent_id"`
	ID             string         `json:"id"`
}

func (q *Queries) SetSessionCurrentAgent(ctx context.Context, arg SetSessionCurrentAgentParams) error {
	_, err := q.exec(ctx, q.setSessionCurrentAgentStmt, setSessionCurrentAgent, arg.CurrentAgentID, arg.ID)
	return err
}
